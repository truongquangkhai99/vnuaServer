...
import com.luantm.vnua.config.KafkaProperties;
...
class VnuaServerKafkaResourceIT {
    ...
    void setup() {
        ...
        VnuaServerKafkaResource kafkaResource = new VnuaServerKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/vnua-server-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/vnua-server-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
